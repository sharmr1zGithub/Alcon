...
public class AdjustInventoryUtil {
  ...
  public String Load_PutAway_Details(String company,String mtid) throws Exception{
    ...
    //query
     String query=" " + MDbConstant.SKU + " , " + MDbConstant.LOT + " , LOC ," + MDbConstant.QTY;
    //condition
    Hashtable ht=new Hashtable();
    ht.put(MDbConstant.COMPANY,company);
    ht.put(MDbConstant.MTID,mtid);
       
  String  shelfLoc= new PoDetDAO().getShelfLocId(company,mtid);
    ...
    try {
      ...
      if (map.size() > 0) {
        //for (int i = 0; i < al.size(); i++) {
         // Map map =  al.get(0);
            xmlStr = xmlStr + xu.getXMLNode("MTID", (String) map.get(MDbConstant.MTID));
            xmlStr = xmlStr + xu.getXMLNode("SKU", (String) map.get(MDbConstant.SKU));
            xmlStr = xmlStr + xu.getXMLNode("LOT", (String) map.get(MDbConstant.LOT));
             xmlStr = xmlStr + xu.getXMLNode("LOC", (String) map.get(MDbConstant.LOT));
            xmlStr = xmlStr + xu.getXMLNode("QTY", (String) map.get(MDbConstant.QTY));
            xmlStr = xmlStr + xu.getXMLNode("SELFLOC", (String) map.get(MDbConstant.WIPMST_UOM));
            xmlStr = xmlStr + xu.getXMLNode("lotSizeQty", lotSizeQty);
         //}
      }
  ...
  public String Load_Details_For_Putaway(String COMPANY, String MTID) {

    MLogger.log(1, this.getClass() + " Load_Details_For_Putaway()");

    String xmlStr = "";
    ArrayList al = null;
    InvMstDAO dao = new InvMstDAO();
    xmlStr = xu.getXMLHeader();
    xmlStr = xmlStr + xu.getStartNode("invDetails");
    try {
  ...
  public String process_Adjust_Inventory(Map obj) {
   ...
   if(flag==true){
    ...
    return xmlStr;
     ...
     private boolean process_Seiko_Adjust_Inventory(Map issueMaterial_HM) throws Exception{
         MLogger.log( 1, this.getClass() + " process_Seiko_Adjust_Inventory()");
         boolean flag=false;
         SeikoTran tran=new com.seiko.tran.SeikoAdjustInventory();
    ...
    private boolean process_Wms_Adjust_Inventory(Map map)  throws Exception{
      MLogger.log(1, this.getClass() + " process_Wms_Adjust_Inventory()");
      boolean flag=false;

      WmsTran tran=new com.murho.tran.WmsAdjustInventory();
