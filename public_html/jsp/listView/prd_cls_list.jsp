...
<%
    ...
    String sBGColor = "";
   try{
    List arrPrdCls = prdclsUtil.getPrdClsListStartsWith(sPrdClsId);
        for (int i =0; i<arrPrdCls.size(); i++){
        Map map = (Map) arrPrdCls.get(i);
        sBGColor = ((i == 0) || (i % 2 == 0)) ? "#FFFFFF" : "#dddddd";
      
        String sPrdClsId1        = (String)map.get("PRD_CLS_ID");
        String sPrdClsDesc1      = strUtils.removeQuotes((String)map.get("PRD_CLS_DESC"));
        String sPrdGrpId1          = (String)map.get("PRD_GRP_ID");

%>
    <TR bgcolor="<%=sBGColor%>">
      <td class="main2"><a href="#" onClick="window.opener.form1.PRD_CLS_ID.disabled = true;window.opener.form1.PRD_CLS_ID.value='<%=sPrdClsId1%>';window.opener.form1.PRD_CLS_DESC.value='<%=sPrdClsDesc1%>';window.opener.form1.PRD_CLS_ID1.value='<%=sPrdClsId1%>';window.opener.form1.PRD_GRP_ID.value='<%=sPrdGrpId1%>';window.close();"><%=sPrdClsId1%></a></td>
